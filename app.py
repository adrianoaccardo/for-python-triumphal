...
def save_wave_np(wave: np.ndarray, sr: int = 22050):
...
def ensure_silero_loaded(language="en", speaker="v3_en"):
    global _silero_loaded, _silero_model, _silero_utils, _silero_sample_rate
    if _silero_loaded:
        return
    if not _TORCH_AVAILABLE:
        raise RuntimeError("torch non disponibile. Installa torch prima di usare Silero TTS.")
    _silero_model, _silero_utils = torch.hub.load(repo_or_dir='snakers4/silero-models',
                                                  model='silero_tts',
                                                  language=language,
                                                  speaker=speaker)
    _silero_sample_rate = 48000
...
def silero_tts(text: str, language="en", speaker="v3_en"):
    ensure_silero_loaded(language=language, speaker=speaker)
    wav = _silero_model.apply_tts(texts=text, speaker=speaker, sample_rate=_silero_sample_rate)
...
def ensure_musicgen_loaded(model_size="small", device="cuda"):
...
def musicgen_generate(prompt: str, duration: int = 8, model_size="small", device="cuda"):
...
with gr.Blocks(title="mini-suno-mvp") as demo:
    ...
    with gr.Tab("Text→Music"):
        prompt = gr.Textbox(label="Prompt musicale", value="Calm ambient pad with soft melody", lines=3)
